...
// Read both pages
const page1Data = JSON.parse(fs.readFileSync('/tmp/storia_data.json', 'utf8'));
const page2Data = JSON.parse(fs.readFileSync('/tmp/storia_data_page2.json', 'utf8'));

const page1Items = page1Data.props.pageProps.searchAds.items || [];
const page2Items = page2Data.props.pageProps.searchAds.items || [];

const allItems = [...page1Items, ...page2Items];

console.log(`Found ${allItems.length} total properties (Page 1: ${page1Items.length}, Page 2: ${page2Items.length})`);
console.log('\nAll properties:\n');

// Transform and display properties
const transformedProperties = allItems.map((item, index) => {
...
// Save to JSON file
fs.writeFileSync('/tmp/all_storia_properties.json', JSON.stringify(transformedProperties, null, 2));
console.log(`✓ Saved ${transformedProperties.length} properties to /tmp/all_storia_properties.json`);
...
transformedProperties.forEach(prop => {
  summary.byType[prop.type] = (summary.byType[prop.type] || 0) + 1;
  summary.byListingType[prop.listingType] = (summary.byListingType[prop.listingType] || 0) + 1;
});

console.log('\nSummary:');
console.log(`  Total: ${summary.total}`);
console.log(`  By Type:`, summary.byType);
console.log(`  By Listing Type:`, summary.byListingType);
